...
interface RawPlayerStat {
  ...
  statistics: Array<{
    team: { id: number };
    games: { appearances: number; minutes: number; position: string };
...
export class APIFootballProvider implements FootballProvider {
  ...
  constructor() {
    ...
    // Direct signup (api-football.com)
  ...
  private async get<T>(
    ...
    if (!res.ok)
      throw new Error(`API-Football HTTP error ${res.status}: ${path}`);
    ...
    if (!Array.isArray(json.response)) {
      throw new Error(
        `API-Football unexpected response shape: ${JSON.stringify(json)}`,
      );
    }
